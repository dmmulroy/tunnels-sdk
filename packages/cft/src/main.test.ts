...
import { cft } from "./main.js"
import { TestLayer } from "./test-layer.js"

const run = Command.runWith(cft, { version: "0.1.0" })

describe("cft root command", () => {
  ...
  it.effect("prints help with --help listing all subcommands", () =>
    Effect.gen(function* () {
      ...
      for (const cmd of ["expose", "tunnel", "ingress", "route", "dns", "vnet", "config", "auth", "status", "dev"]) {
