...
const run = Command.runWith(tunnel, { version: "0.1.0" })
...
describe("cft tunnel", () => {
  describe("create", () => {
    it.effect("creates a tunnel by name", () =>
      Effect.gen(function* () {
        ...
        yield* run(["create", "my-tunnel"]).pipe(
    ...
    it.effect("creates a tunnel with --dns flag", () =>
      Effect.gen(function* () {
        const { service, getCreated } = yield* makeTestTunnelApi
        yield* run(["create", "my-tunnel", "--dns"]).pipe(
          Effect.provideService(TunnelApiService, service)
        )
        const created = yield* getCreated
  ...
  describe("list", () => {
    ...
    it.effect("filters tunnels by --status", () =>
      Effect.gen(function* () {
        const { service } = yield* makeTestTunnelApi
        yield* run(["list", "--status", "healthy"]).pipe(
          Effect.provideService(TunnelApiService, service),
          Effect.provideService(OutputContext, defaultOutputContext)
        )
        const output = yield* TestConsole.logLines
  ...
  describe("delete", () => {
    ...
    it.effect("deletes a tunnel with --force", () =>
      Effect.gen(function* () {
        const { service, getDeleted } = yield* makeTestTunnelApi
        yield* run(["delete", "my-app", "--force"]).pipe(
          Effect.provideService(TunnelApiService, service)
        )
        const deleted = yield* getDeleted
  ...
  describe("error handling", () => {
    it.effect("info shows error when tunnel is not found", () =>
      Effect.gen(function* () {
        const service = TunnelApiService.of({
          ...makeNoopApi(),
          get: (ref) => Effect.fail(CftError.NetworkError({ message: `Tunnel not found: "${ref}"` })),
        })
        const result = yield* run(["info", "nonexistent"]).pipe(
          Effect.provideService(TunnelApiService, service),
          Effect.exit
        )
    ...
    it.effect("list shows error on network failure", () =>
      Effect.gen(function* () {
        const service = TunnelApiService.of({
          ...makeNoopApi(),
          list: () => Effect.fail(CftError.NetworkError({ message: "Connection refused" })),
        })
        const result = yield* run(["list"]).pipe(
          Effect.provideService(TunnelApiService, service),
          Effect.provideService(OutputContext, defaultOutputContext),
          Effect.exit
        )
