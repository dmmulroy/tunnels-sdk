...
You are working on refactoring the `tunnels` package from class-based async/await TypeScript to idiomatic Effect TS. The full design lives in `EFFECT_REFACTOR_PLAN.md` at the repo root. The work is tracked as file-backed todos tagged `effect-refactor`.
...
5. **When done**: verify `pnpm typecheck` and `pnpm test` pass in `packages/tunnels`, then close the todo.
...
Config validation    CloudflareApi svc   CloudflaredBinary svc
        ...
        Remove old code   Wire tunnels CLI   Update examples
```
...
- **Current SDK source**: `packages/tunnels/src/` — the existing implementation you're porting from
- **Current SDK tests**: `packages/tunnels/src/**/*.test.ts` — behaviors to preserve
...
- Run `pnpm typecheck` and `pnpm test` in `packages/tunnels` before closing a todo.
